...
/**
 * PRAMOGŲ NUKREIPIMO PUSLAPIS (/pramogos)
 * Skirtas Google Ads kampanijai – į jį nėra nuorodų iš pagrindinio meniu.
 * Orientuotas į apsilankymą su šeima ar draugais.
 */
...
const STEPS: [string, string, string][] = [
  ['01', 'Atvykstate ir apmokome', 'Pasitinkame, pritaikome VR įrangą ir per 5 minutes paruošiame visus žaidimui – tinka ir pirmą kartą bandantiems.'],
  ['02', 'Neriate į nuotykį komandoje', 'Adrenalinas, juokas ir šūksniai – iki 5 žmonių komandoje laisvai judate po kambarį, kovojate ir padedate vieni kitiems realiame laike.'],
  ['03', 'Dalinatės emocijomis', 'Po žaidimo – pats smagiausias momentas: aptariate, kas pavyko, juokiatės iš nutikimų ir iškart norite žaisti dar kartą.'],
];

const FACTS: [string, string][] = [
  ['Amžius', 'Nuo 8 metų – VR nuotykių žaidimai šeimai ir draugams'],
...
export default function VisitPage() {
  const onCall = () => {
    trackEvent('phone_click', { page: 'pramogos' });
    trackPixel('Contact', { content_name: 'pramogos' });
  };
  ...
  return (
    <div style={{ background: 'var(--bg-stage)', minHeight: '100vh', color: '#fff' }}>
      ...
      <section style={{ textAlign: 'center', padding: '56px 24px 64px', maxWidth: 880, margin: '0 auto' }}>
        <Kicker>VR pramogos · Klaipėda</Kicker>
        <h1 style={{
          font: '800 clamp(34px, 7vw, 72px)/1.02 var(--font-display)',
          textTransform: 'uppercase', letterSpacing: '-0.01em', margin: '14px 0 20px',
        }}>
          Nuotykis šeimai<br />
          <span style={{ background: 'var(--heat-gradient)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>ir draugams</span>
        </h1>
        <p style={{ font: '400 17px/1.6 var(--font-body)', color: 'rgba(255,255,255,0.78)', maxWidth: 560, margin: '0 auto 32px' }}>
          Adrenalinas, juokas ir šūksniai – kartu kovokite su zombiais, gelbėkite pasaulį
          ir nugalėkite vieni kitus virtualioje arenoje. Veiksmo nuotykis, kurį prisiminsite ilgai.
        </p>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          {[['20 €', 'asmeniui'], ['50 min', 'nuotykių'], ['Iki 5', 'žmonių'], ['Green Sq.', 'Klaipėda']].map(([v, l]) => (
            <div key={v} style={{
      ...
      <section style={{ background: 'var(--bg-surface)', color: 'var(--fg)', borderRadius: '32px 32px 0 0', padding: '64px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <Kicker>Kaip vyksta apsilankymas</Kicker>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            ...
            <p style={{ font: '400 15px/1.6 var(--font-body)', color: 'var(--fg-muted)', maxWidth: 520, margin: '0 auto' }}>
              Per apsilankymą komanda spėja išbandyti kelis žaidimus – nuo linksmų rungčių iki komandinių nuotykių.
            </p>
          </div>
      ...
      <section style={{ background: 'var(--off-white)', color: 'var(--fg)', padding: '64px 24px' }}>
        <div style={{ maxWidth: 880, margin: '0 auto' }}>
          ...
          <p style={{ textAlign: 'center', font: '400 14px/1.6 var(--font-body)', color: 'var(--fg-muted)', marginTop: 28 }}>
            VR pabėgimo kambariai puikiai tinka tiek šeimoms su vaikais, tiek draugų kompanijoms
            ir įvairioms šventėms. Paskambinkite – suplanuosime kartu.
          </p>
        </div>
      </section>
      ...
      <section style={{ textAlign: 'center', padding: '72px 24px 80px', maxWidth: 720, margin: '0 auto' }}>
        <h2 style={{ font: '800 clamp(28px, 5vw, 48px)/1.05 var(--font-display)', textTransform: 'uppercase', margin: '0 0 16px' }}>
          Rezervuokite apsilankymą
        </h2>
        <p style={{ font: '400 16px/1.6 var(--font-body)', color: 'rgba(255,255,255,0.75)', margin: '0 0 32px' }}>
          Savaitgaliai ir vakarai pildosi greitai – aptarkime laiką, žmonių skaičių ir žaidimus telefonu,
